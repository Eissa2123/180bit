	...
	<div id="content">
        <div class="row">
            <div class="col-xs-12 col-sm-9 col-md-10 col-lg-10">
                <ol class="breadcrumb">
                    ...
                    <li><a href="<?= WLink('purchase') ?>"><?= V($LTranslates, 'Purchase') ?></a></li>
                    <li><a href="<?= WLink('purchase/display/'.V($LPosts, 'Purchase')) ?>"><?= V($LPosts, 'Purchase') ?></a></li>
                    <li><a href="<?= WLink('purchasepayment/'.V($LPosts, 'Purchase')) ?>"><?= V($LTranslates, 'Payments') ?></a></li>
                    <li class="active"><?= V($LTranslates, 'Add') ?></li>
                </ol>
            </div>
            <div class="col-xs-12 col-sm-3 col-md-2 col-lg-2">
                <a href="<?= WLink('purchasepayment/'.V($LPosts, 'Purchase')) ?>" class="btn btn-default btn-block btn-control" role="button">
                    <?= V($LTranslates, 'List') ?> 
        ...
        <div class="panel panel-default">
            <div class="panel-body">
                <form action="<?= WLink('purchasepayment/add/'.V($LPosts, 'Purchase')) ?>" method="post">  
                    <div class="row">
                        <div class="col-xs-12 col-sm-4 col-md-3 col-lg-3">
                            <div class="form-group">
                                <input type="hidden" class="form-control" name="purchase" value="<?= V($LPosts, 'Purchase') ?>" />
                        ...
                        <div class="col-xs-12 col-sm-4 col-md-3 col-lg-3">
                            <div class="form-group">
                                <label><?= V($LTranslates,'Purchase') ?></label>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="provider" required="required">
                                    ...
                                    <?php if (isset($LProviders)){  ?>
                                        ...
                                        <option value="<?= V($Provider,'ID') ?>" <?= (V($Cells,'Provider') == V($Provider,'ID')) ? 'selected="selected"' : '' ?>><?= V($Provider,'Code') ?> - <?= V($Provider,'Companyname') ?> <?= V($Provider,'Name') ?> : <?= V($Provider,'Job') ?></option>
                                        <?php }  ?>
                                    <?php }  ?>
                                </select>
                            </div>
