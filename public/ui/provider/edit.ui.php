	...
	<div id="content">
        ...
        <div class="panel panel-default">
            <div class="panel-body">
				<form action="<?= WLink('provider/edit/'.V($Cells, 'ID')) ?>" method="post" enctype="multipart/form-data">
                   <div class="row">
						...
						<div class="col-xs-12 col-sm-8 col-md-10 col-lg-10">
							...
							<div class="row">
                                ...
                                <div class="col-xs-12 col-sm-12 col-md-3 col-lg-3">
                                    <div class="form-group">
                                        <label for="terms"><?= V($LTranslates, 'Categories') ?></label>
                                        <select class="form-control multiselect" id="multiselect-categories" name="categories[]" multiple="multiple" data-multiple-name="<?= V($LTranslates,'Categories') ?>" data-multiple-nselected="<?= V($LTranslates,'NSelectedText') ?>" data-multiple-allselected="<?= V($LTranslates,'AllSelectedText') ?>" data-multiple-selectall="<?= V($LTranslates,'SelectAll') ?>">
                                            
                                            <?php if (isset($LCategories)){  ?>
                                                <?php foreach ($LCategories as $Category) { ?>
                                                <option value="<?= V($Category,'ID') ?>" <?= I($Category, 'ID', $Cells, 'Categories') ? 'selected="selected"' : '' ?>>
                                                    <?= V($Category,'Name') ?>
                                                </option>
                                                <?php }  ?>
                                            <?php }  ?>
