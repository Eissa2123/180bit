	...
	<div id="content">
        <div class="row">
            <div class="col-xs-12 col-sm-9 col-md-8 col-lg-8">
                <ol class="breadcrumb">
                    ...
                    <li class="active"><?= V($LTranslates, 'Providers') ?></li>
                </ol>
            </div>
            <div class="col-xs-12 col-sm-3 col-md-2 col-lg-2">
                <a href="<?= WLink('provider/add') ?>" class="btn btn-default btn-block btn-control" role="button">
                    <?= V($LTranslates, 'Add') ?>  
                    <i class="glyphicon glyphicon-plus"></i> 
                </a>
            </div>
            <div class="col-xs-12 col-sm-3 col-md-2 col-lg-2">
                <form action="<?= WLink('provider/prints') ?>" method="post" target="_blank">
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-header">
                <div class="row">
                    <form action="<?= WLink('provider') ?>" method="post">
                    ...
                    <div class="row">
                        ...
                        <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
                            <a href="<?= WLink('provider') ?>" class="btn btn-danger btn-block btn-control" role="button">
                                <?= V($LTranslates,'Clean') ?>  
			...
			<?php if (isset($LProviders)){  ?>
                <table class="table table-bordered">
                    ...
                    <tbody>
                        ...
                        <tr class="<?= V($provider,'State', 'Class') ?>">
                            <td class="text-center"><?= V($provider,'ID') ?></td> 
                            <td class="text-center hidden-xs"><?= V($provider,'Code') ?></td> 
                            <td class="text-center"><?= V($provider, 'RC') ?></td>
                            <td class="text-center"><?= V($provider, 'Taxnumber') ?></td>
                            <td class="text-center"><?= V($provider, 'Companyname') ?></td>
                            <td class="text-center hidden-xs"><span class="label label-<?= V($provider,'State', 'Class') ?>"><?= V($provider,'State','Name'.LNG) ?></span></td> 
                            <td class="text-center actions">
                                <a href="<?= WLink('provider/print/'.V($provider,'ID')) ?>" title="<?= V($LTranslates,'Print') ?>" target="_blank"><span class="glyphicon glyphicon-print"></span></a>	
                                <a href="<?= WLink('provider/display/'.V($provider,'ID')) ?>" title="<?= V($LTranslates,'Display') ?>"><span class="glyphicon glyphicon-eye-open"></span></a>	
                                <a href="<?= WLink('provider/edit/'.V($provider,'ID')) ?>" title="<?= V($LTranslates,'Edit') ?>"><span class="glyphicon glyphicon-edit"></span></a>	
                                <a href="<?= WLink('provider/remove/'.V($provider,'ID')) ?>" title="<?= V($LTranslates,'Remove') ?>"><span class="glyphicon glyphicon-trash"></span></a>	
                            </td>
                        </tr>
                        <?php } ?>
                    </tbody>
