	...
	<div id="content">
        <div class="row">
            <div class="col-xs-12 col-sm-9 col-md-8 col-lg-8">
                <ol class="breadcrumb">
                    ...
                    <li class="active"><?= V($LTranslates, 'BalanceFinancials') ?></li>
                </ol>
            </div>
            <div class="col-xs-12 col-sm-3 col-md-2 col-lg-2">
                <a href="<?= WLink('balancefinancial/add') ?>" class="btn btn-default btn-block btn-control" role="button">
                    <?= V($LTranslates, 'Add') ?>  
                    <i class="glyphicon glyphicon-plus"></i> 
                </a>
            </div>
            <div class="col-xs-12 col-sm-3 col-md-2 col-lg-2">
                <form action="<?= WLink('balancefinancial/prints') ?>" method="post" target="_blank">
        ...
        <div class="panel panel-default">
            <div class="panel-header">
                <div class="row">
                    <form action="<?= WLink('balancefinancial') ?>" method="post">
                    ...
                    <div class="row">
                        ...
                        <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
                            <a href="<?= WLink('balancefinancial') ?>" class="btn btn-danger btn-block btn-control" role="button">
                                <?= V($LTranslates,'Clean') ?>  
			...
			<?php if (isset($LRevenues)){  ?>
                <table class="table table-bordered table-striped">
                    <thead>
                        ...
                        <th class="text-center"><?= V($LTranslates,'Notes') ?></th>
                        ...
                        <th class="text-center"><?= V($LTranslates,'Employee') ?></th>
                    ...
                    <tbody>
                        ...
                        <tr class="<?= V($revenue,'Method') === 'C' ? 'success' : 'warning' ?>">
                            <td class="text-center"><?= V($revenue,'ID') ?></td> 
                            <td class="text-center hidden-xs"><?= V($revenue,'Code') ?></td> 
                            <td class="text-center"><?= V($revenue, 'Name') ?></td>
                            <td class="text-center"><?= V($revenue, 'AT') ?> </td>
                            <td class="text-left"><?= V($revenue, 'Amount') ?> </td>
                            <td class="text-center ">
                                <?php if(V($revenue, 'Method') == 'B'){ ?>
                                    <?= V($LTranslates, 'Bank') ?> 
                                <?php }else if(V($revenue, 'Method') == 'C'){ ?>
                                    <?= V($LTranslates, 'Cashbox') ?> 
                                <?php } ?>
                            </td>
                            <td class="text-center"><?= V($revenue, 'Employee', 'Firstname') ?> <?= V($revenue, 'Employee', 'Lastname') ?></td>
                            <td class="text-center actions">
                                <a href="<?= WLink('balancefinancial/print/'.V($revenue,'ID')) ?>" title="<?= V($LTranslates,'Print') ?>" target="_blank"><span class="glyphicon glyphicon-print"></span></a>	
                                <a href="<?= WLink('balancefinancial/display/'.V($revenue,'ID')) ?>" title="<?= V($LTranslates,'Display') ?>"><span class="glyphicon glyphicon-eye-open"></span></a>	
                                <a href="<?= WLink('balancefinancial/edit/'.V($revenue,'ID')) ?>" title="<?= V($LTranslates,'Edit') ?>"><span class="glyphicon glyphicon-edit"></span></a>	
                                <a href="<?= WLink('balancefinancial/remove/'.V($revenue,'ID')) ?>" title="<?= V($LTranslates,'Remove') ?>"><span class="glyphicon glyphicon-trash"></span></a>	
                            </td>
                        </tr>
                        <?php } ?>
                    </tbody>
