	...
	<div id="content">
        ...
        <div class="panel panel-default">
            <div class="panel-body">
				<form action="<?= WLink('historique/edit/'.V($Cells, 'ID')) ?>" method="post">
                   <div class="row">
						<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
						<div class="alert alert-warning" role="alert"><?= V($LTranslates, 'MRecovery') ?></div>
							<div class="form-group">
								<input type="hidden" class="form-control" name="id" value="<?= V($Cells, 'ID') ?>" />

								<label for="gender"><?= V($LTranslates,'Gender') ?></label>
								<input type="radio" name="before" value="1" checked="checked" /> <?= V($LTranslates,'Before') ?>
								<input type="radio" name="before" value="0" /> <?= V($LTranslates,'After') ?>
							</div>
						</div>
					</div>
                    <hr />
