	...
	<div id="content">
        <div class="row">
            <div class="col-xs-12 col-sm-9 col-md-8 col-lg-8">
                <ol class="breadcrumb">
                    ...
                    <li class="active"><?= V($LTranslates, 'Marketers') ?></li>
                </ol>
            </div>
            <div class="col-xs-12 col-sm-3 col-md-2 col-lg-2">
                <a href="<?= WLink('marketer/add') ?>" class="btn btn-default btn-block btn-control" role="button">
                    <?= V($LTranslates, 'Add') ?>  
                    <i class="glyphicon glyphicon-plus"></i> 
                </a>
            </div>
            <div class="col-xs-12 col-sm-3 col-md-2 col-lg-2">
                <form action="<?= WLink('marketer/prints') ?>" method="post" target="_blank">
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-header">
                <div class="row">
                    <form action="<?= WLink('marketer') ?>" method="post">
                    ...
                    <div class="row">
                        ...
                        <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
                            <a href="<?= WLink('marketer') ?>" class="btn btn-danger btn-block btn-control" role="button">
                                <?= V($LTranslates,'Clean') ?>  
			...
			<?php if (isset($LMarketers)){  ?>
                <table class="table table-bordered">
                    <thead>
                        ...
                        <th class="text-center"><?= V($LTranslates,'Country') ?></th>
                        <th class="text-center"><?= V($LTranslates,'City') ?></th>
                        <th class="text-left"><?= V($LTranslates,'Due') ?></th>
                        <th class="text-left"><?= V($LTranslates,'Paid') ?></th>
                        <th class="text-left"><?= V($LTranslates,'Remaining') ?></th>
                    ...
                    <tbody>
                        ...
                        <tr class="<?= V($marketer,'State', 'Class') ?>">
                            <td class="text-center"><?= V($marketer,'ID') ?></td> 
                            <td class="text-center hidden-xs"><?= V($marketer,'Code') ?></td> 
                            <td class="text-center"><?= V($marketer, 'Name') ?></td>
                            <td class="text-center"><?= V($marketer, 'Country') ?></td>
                            <td class="text-center"><?= V($marketer, 'City') ?></td>
                            <td class="text-left"><?= V($marketer, 'Due') ?></td>
                            <td class="text-left"><?= V($marketer, 'Paid') ?></td>
                            <td class="text-left"><?= V($marketer, 'Remaining') ?></td>
                            <td class="text-center hidden-xs"><span class="label label-<?= V($marketer,'State', 'Class') ?>"><?= V($marketer,'State','Name'.LNG) ?></span></td> 
                            <td class="text-center actions">
                                <a href="<?= WLink('marketer/print/'.V($marketer,'ID')) ?>" title="<?= V($LTranslates,'Print') ?>" target="_blank"><span class="glyphicon glyphicon-print"></span></a>	
                                <a href="<?= WLink('marketer/display/'.V($marketer,'ID')) ?>" title="<?= V($LTranslates,'Display') ?>"><span class="glyphicon glyphicon-eye-open"></span></a>	
                                <a href="<?= WLink('marketer/edit/'.V($marketer,'ID')) ?>" title="<?= V($LTranslates,'Edit') ?>"><span class="glyphicon glyphicon-edit"></span></a>	
                                <a href="<?= WLink('marketer/remove/'.V($marketer,'ID')) ?>" title="<?= V($LTranslates,'Remove') ?>"><span class="glyphicon glyphicon-trash"></span></a>	
                            </td>
                        </tr>
                        <?php } ?>
                    </tbody>
