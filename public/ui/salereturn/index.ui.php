        ...
        <div class="row">
            <div class="col-xs-12 col-sm-9 col-md-8 col-lg-8">
                <ol class="breadcrumb">
                    ...
                    <li><a href="<?= WLink('sale/display/'.V($LPosts, 'Facture')) ?>"><?= V($LTranslates, 'Sale') ?></a></li>
                    <li class="active"><?= V($LTranslates, 'Returns') ?></li>
                </ol>
            </div>
            <div class="col-xs-12 col-sm-3 col-md-2 col-lg-2">
                <a href="<?= WLink('salereturn/add/'.V($LPosts, 'Facture')) ?>" class="btn btn-default btn-block btn-control" role="button">
                    <?= V($LTranslates, 'Add') ?>   
                    <i class="glyphicon glyphicon-plus"></i> 
                </a>
            </div>
            <div class="col-xs-12 col-sm-3 col-md-2 col-lg-2">
                <form action="<?= WLink('salereturn/prints') ?>" method="post" target="_blank">
        ...
        <div class="panel panel-default">
            <div class="panel-body" id="repport">
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <?php foreach ($LSalereturns as $Salereturn) { ?>
                        <tr>
                            <td class="text-center hidden-xs"><?= V($Salereturn,'ID') ?></td> 
                            <td class="text-center"><?= V($Salereturn,'Code') ?></td> 
                            <td class="text-center hidden-xs hidden-sm"><?= V($Salereturn,'AT') ?></td> 
                            <td class="text-left hidden-xs"><?= V($Salereturn,'TTC') ?></td>
                            <td class="text-center actions">
                                <a href="<?= WLink('salereturn/print/'.V($Salereturn,'ID')) ?>" title="<?= V($LTranslates,'Print') ?>" target="_blank"><span class="glyphicon glyphicon-print"></span></a>
                                <a href="<?= WLink('salereturn/display/'.V($Salereturn,'ID')) ?>" title="<?= V($LTranslates,'Display') ?>"><span class="glyphicon glyphicon-eye-open"></span></a>	
                                <?php /*
                                <a href="<?= WLink('salereturn/edit/'.V($Salereturn,'ID')) ?>" title="<?= V($LTranslates,'Edit') ?>"><span class="glyphicon glyphicon-edit"></span></a>	
                                <a href="<?= WLink('salereturn/remove/'.V($Salereturn,'ID')) ?>" title="<?= V($LTranslates,'Remove') ?>"><span class="glyphicon glyphicon-trash"></span></a>*/
                                ?>	
                            </td>
                        </tr>
                        <?php } ?>
