	...
	<div id="content">
        <div class="row">
            <div class="col-xs-12 col-sm-9 col-md-8 col-lg-8">
                <ol class="breadcrumb">
                    ...
                    <li class="active"><?= V($LTranslates, 'Cobons') ?></li>
                </ol>
            </div>
            <div class="col-xs-12 col-sm-3 col-md-2 col-lg-2">
                <a href="<?= WLink('cobon/add') ?>" class="btn btn-default btn-block btn-control" role="button">
                    <?= V($LTranslates, 'Add') ?>  
                    <i class="glyphicon glyphicon-plus"></i> 
                </a>
            </div>
            <div class="col-xs-12 col-sm-3 col-md-2 col-lg-2">
                <form action="<?= WLink('cobon/prints') ?>" method="post" target="_blank">
        ...
        <div class="panel panel-default">
            <div class="panel-header">
                <div class="row">
                    <form action="<?= WLink('cobon') ?>" method="post">
                                ...
                                <select class="form-control multiselect" id="multiselect-marketers" name="marketers[]" multiple="multiple" data-multiple-name="<?= V($LTranslates,'Marketer') ?>" data-multiple-nselected="<?= V($LTranslates,'NSelectedText') ?>" data-multiple-allselected="<?= V($LTranslates,'AllSelectedText') ?>" data-multiple-selectall="<?= V($LTranslates,'SelectAll') ?>">
                                    ...
                                    <?php if (isset($LMarketers)){  ?>
                                        ...
                                        <option value="<?= V($Marketer,'ID') ?>" <?= I($Marketer, 'ID', $LPosts,'Marketers') ? 'selected="selected"' : '' ?>>
                                            <?= V($Marketer,'Companyname') ?> : <?= V($Marketer,'Name') ?>
                                        </option>
                                        <?php }  ?>
                                    <?php }  ?>
                    ...
                    <div class="row">
                        ...
                        <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
                            <a href="<?= WLink('cobon') ?>" class="btn btn-danger btn-block btn-control" role="button">
                                <?= V($LTranslates,'Clean') ?>  
			...
			<?php if (isset($LCobons)){  ?>
                <table class="table table-bordered">
                    <thead>
                        ...
                        <th class="text-center"><?= V($LTranslates,'Marketer') ?></th>
                        <th class="text-center"><?= V($LTranslates,'Ratio') ?></th>
                        <th class="text-center"><?= V($LTranslates,'Ratios') ?></th>
                    ...
                    <tbody>
                        ...
                        <tr class="<?= V($cobon,'State', 'Class') ?>">
                            <td class="text-center hidden-xs"><?= V($cobon,'Code') ?></td> 
                            <td class="text-center"><?= V($cobon, 'Name') ?></td>
                            <td class="text-center"><?= V($cobon, 'Marketer', 'Name') ?></td>
                            <td class="text-center"><?= V($cobon, 'Ratio') ?> %</td>
                            <td class="text-center"><?= V($cobon, 'Ratios') ?> %</td>
                            <td class="text-center hidden-xs"><span class="label label-<?= V($cobon,'State', 'Class') ?>"><?= V($cobon,'State','Name'.LNG) ?></span></td> 
                            <td class="text-center actions">
                                <a href="<?= WLink('cobon/print/'.V($cobon,'ID')) ?>" title="<?= V($LTranslates,'Print') ?>" target="_blank"><span class="glyphicon glyphicon-print"></span></a>	
                                <a href="<?= WLink('cobon/display/'.V($cobon,'ID')) ?>" title="<?= V($LTranslates,'Display') ?>"><span class="glyphicon glyphicon-eye-open"></span></a>	
                                <a href="<?= WLink('cobon/edit/'.V($cobon,'ID')) ?>" title="<?= V($LTranslates,'Edit') ?>"><span class="glyphicon glyphicon-edit"></span></a>	
                                <a href="<?= WLink('cobon/remove/'.V($cobon,'ID')) ?>" title="<?= V($LTranslates,'Remove') ?>"><span class="glyphicon glyphicon-trash"></span></a>	
                            </td>
                        </tr>
                        <?php } ?>
                    </tbody>
