<?php
   class SalequotationCore extends Core{
      ...
      public function Customer($value = null) {
         if($value === null) {
            ...
            $this->_Customer =  (is_integer((int) $value) and ((int) $value) > 0) ? ((int) $value) : CNF_ZER;
         }
      }
         ...
         if($values === null) {
            ...
            if(is_array($values) and count($values) > 0){
               foreach($values as $value){
                  ...
                  $Product['Quantity'] = isset($value['quantity']) ? (double) $value['quantity'] : CNF_ZER;
