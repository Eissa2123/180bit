   ...
   class PurchasereturnModel extends Model{
      ...
      public function index($in){
         ...
         if($sale == null){
            ...
            if(is_array($send['LPurchasereturns']) and count($send['LPurchasereturns']) > 0){
               foreach($send['LPurchasereturns'] as $KPurchase => $VPurchase){
                  ...
                  $send['LPurchasereturns'][$KPurchase] = $VPurchase;

                  $send['TPR']['Total'] += (double) $VPurchase['TTC'];
               }
            }
         }
      ...
      public function insert($in){
         ...
         if(!L($send, 'Errors')){
            ...
            $send['Success'] = $this->into(Model::PURCHASE_RETURN)->set($Facture)->save();
            if($send['Success'] === true){
               ...
               foreach($Products as $KProduct => $VProduct){
                  ...
                  $r = $this->into(Model::PURCHASE_RETURN_DETAIL)->set($VProduct)->save();
               }
      ...
      public function update($in){
         ...
         if(!isset($send['Errors'])){
            ...
            $Facture = $in;
            unset($Facture['Products']);
            $send['Success'] = $this->into(Model::PURCHASE_RETURN)->apt($Facture)->save();

            if($send['Success'] === true){

               $this->into(Model::PURCHASE_RETURN_DETAIL)->del($in['ID'], true, 'Facture')->save();
               ...
               foreach($Products as $KProduct => $VProduct){
                  ...
                  $this->into(Model::PURCHASE_RETURN_DETAIL)->set($VProduct)->save();
               }
            }
         }
      ...
      public function delete($in){
         ...
         if(!isset($send['Errors'])){
            $send['Success'] = $this->into(Model::PURCHASE_RETURN)->del($in)->save();
            if($send['Success'] === true){
               $this->into(Model::PURCHASE_RETURN_DETAIL)->del($in['ID'], false, 'Facture')->save();
            }
            $send['Cells'] = $this->select('*')
               ->from(Model::PURCHASE_RETURN)
               ->where('ID', $in['ID'])
